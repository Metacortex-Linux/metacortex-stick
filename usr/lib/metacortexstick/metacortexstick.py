...
if Using_Unity:
    launcher = Unity.LauncherEntry.get_for_desktop_id("metacortexstick.desktop")

APP = 'metacortexstick'
LOCALE_DIR = "/usr/share/metacortex/locale"
...
class MetacortexStick:
    def __init__(self, iso_path_arg=None, usb_path_arg=None, filesystem_arg=None, mode_arg=None, debug_arg=False):
        ...
        # get glade tree
        self.gladefile = "/usr/share/metacortexstick/metacortexstick.ui"
    ...
    def raw_format(self, usb_path_arg, fstype, label):
        if os.geteuid() > 0:
            polkit_exec = 'pkexec'
            self.process = Popen(
                [polkit_exec, '/usr/bin/python3', '-u', '/usr/lib/metacortexstick/raw_format.py', '-d', usb_path_arg,
                 '-f', fstype, '-l', label, '-u', str(os.geteuid()), '-g', str(os.getgid())],
                shell=False, stdout=PIPE, preexec_fn=os.setsid)
        else:
            self.process = Popen(
                ['/usr/bin/python3', '-u', '/usr/lib/metacortexstick/raw_format.py', '-d', usb_path_arg,
                 '-f', fstype, '-l', label, '-u', str(os.geteuid()), '-g', str(os.getgid())],
                shell=False, stdout=PIPE, preexec_fn=os.setsid)
    ...
    def raw_write(self, source, target):
        ...
        if os.geteuid() > 0:
            polkit_exec = 'pkexec'
            self.process = Popen(
                [polkit_exec, '/usr/bin/python3', '-u', '/usr/lib/metacortexstick/raw_write.py', '-s', source, '-t', target],
                shell=False, stdout=PIPE, preexec_fn=os.setsid)
        else:
            self.process = Popen(
                ['/usr/bin/python3', '-u', '/usr/lib/metacortexstick/raw_write.py', '-s', source, '-t', target],
                shell=False, stdout=PIPE, preexec_fn=os.setsid)
...
if __name__ == "__main__":
    ...
    def usage():
        print("Usage: metacortexstick [--debug] -m [format|iso]              : mode (format usb stick or burn iso image)")
        print("       metacortexstick [--debug] -m iso [-i|--iso] iso_path")
        print("       metacortexstick [--debug] -m format [-u|--usb] usb_device ")
    ...
    MetacortexStick(iso_path, usb_path, filesystem, mode, debug)
